...
（[免責事項](#免責事項)を参照）。`--experimental` を付けると、これに加えて外部 MIDI コントローラー割り当てパネル（**デバイス → MIDI コントロール**。
実機コントローラーでの検証待ち）と破壊・復元方式のセルフテスト診断が有効になる。
...
既定では非表示の機能（外部 MIDI コントロールパネルと実機セルフテスト診断）を有効にするには
`--experimental` を付けて起動する:
