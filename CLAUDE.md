...
- `src/models/` — 機種定義。`build.ts` が機種パラメータから `DeviceModel` (nodes + 接続規則) を生成。`index.ts` が URX22/44/44V を登録。`initial-state.ts` の `defaultPlan` が新規プランの初期値を生成 (実機キャプチャ済み機種は工場初期値をシード。データは `initial-urx44v.ts`)
