...
- **実機側操作の追従 (device follow)** — ライブ同期の逆方向。同じ排出経路で**実機側のパラメーター変更**も
  ...
  `follow: "direct"`) は読み戻しなしで値を直接 plan へ適用し (`applyDirect`・反映は共有の反映ファネルにまとめる)、
  ...
  だけを読み戻して盤面へ追従する。反映は direct・読み戻しとも同じファネルを通し、約 20 Hz (実機は約 10 Hz) に
  上限を設ける。可視なビューは常に 1 つだけなので、非表示ビューの再構築は次に開くまで遅延させる。アプリ自身の
  書込みの戻りは snapshot 一致で無視し、登録アドレス集合は構造変更時のみ再登録する。
